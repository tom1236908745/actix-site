...
/// original body
async fn response_original_body(path: web::Path<String>) -> HttpResponse {
    let name = path.into_inner();

    HttpResponse::Ok().streaming(stream! {
        yield Ok::<_, Infallible>(web::Bytes::from("hey "));
        yield Ok::<_, Infallible>(web::Bytes::from("men! "));
        yield Ok::<_, Infallible>(web::Bytes::from(name));
    })
}
...
async fn main() -> io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            ...
            .service(web::resource("/hey-men/{name}").route(web::get().to(response_original_body)))
            // default
